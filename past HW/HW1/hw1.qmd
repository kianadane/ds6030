...
testing <- tibble(
  ...
  err = rnorm(n_test, mean = 0, sd = sigma),
  Y = -1 + 0.5 * X + 0.2 * X^2 + err) %>%
  select(-err)
...
for (i in 1:n_training) {
  training <- tibble(
    X = rnorm(n_samps3, mean = 0, sd = 1),
    err = rnorm(n_samps3, mean = 0, sd = st_dev),
    Y = -1 + 0.5 * X + 0.2 * X^2 + err
  ) %>%
    select(-err)
  
  lin_m <- lm(Y ~ X, data = training)
  quad_m <- lm(Y ~ poly(X, 2, raw = TRUE), data = training)
  cube_m <- lm(Y ~ poly(X, 3, raw = TRUE), data = training)

# all MSE's
  linear_mse[i] <- mean((testing2$Y - predict(lin_m, newdata = testing2))^2)
  quad_mse[i] <- mean((testing2$Y - predict(quad_m, newdata = testing2))^2)
  cubic_mse[i] <- mean((testing2$Y - predict(cube_m, newdata = testing2))^2)
}

cat("Avg MSE for Linear Model:", mean(linear_mse), "\n")
cat("Avg MSE for Quad Model:", mean(quad_mse))
cat("Avg MSE for Cubic Model:", mean(cubic_mse), "\n")
...
ggplot(mse_results, aes(x = test_MSE, fill = model)) +
  ...
  labs(title = "Kernel Density Plot of MSE Vals",
       ...
       y = "Density") +
  theme_minimal() +
  scale_fill_manual(values = c("Linear" = "navy", "Quadratic" = "red", "Cubic" = "yellow"))
```
